...
## Our warfarin data file: a simple lattice plot to provide an impression
xyplot(
...
## Subjects with and without information on absorption on day 1
xyplot(
  DV ~ TIME|Group,         #plot DV by TIME
  groups = ID,       #make separate curves by ID
  data = PKdata,     #use PKdata
  type = 'l',        #only plot a line"
  lwd = 2,           #make the lines '2' wide
...
## A closer look at the information on absorption on day 1
xyplot(
  DV ~ TIME,         #plot DV by TIME
  groups = ID,       #make separate curves by ID
  data = PKdata[TIME<=24&SPARSE==0],     #use PKdata
  type = 'l',        #only plot a line"
  lwd = 2,           #make the lines '2' wide
  xlab = "Time (h)", #x-axis label
  ylab = "Warfarin (mg/L)" #y-axis label
...
ggplot(data = PKdata, aes(x = TIME, y = DV)) +
  ...
  labs(y = "Warfarin (mg/L)", color = "Group") +
  theme(legend.position = "none") +
  facet_grid(.~Group) 

## Focusing on absorption: first 24 hours of richly sampled profiles on linear scale
ggplot(data = PKdata[TIME<=24&SPARSE==0], aes(x = TIME, y = DV)) +
  geom_line(aes(group = ID), color = "grey50", size = 1, alpha = 0.3) +
  xgx_geom_ci(aes(x = TIME, color = NULL, group = NULL, shape = NULL), conf_level = 0.95) +
  #xgx_scale_y_log10() +
  xgx_scale_x_time_units(units_dataset = "hours", units_plot = "hours") +
  labs(y = "Warfarin (mg/L)", color = "Group") +
  theme(legend.position = "none") +
  facet_grid(.~Group) 


## Or with median and 95% of the data
ggplot(data = PKdata[TIME<=24&SPARSE==0], aes(x = TIME, y = DV)) +
  geom_line(aes(group = ID), color = "grey50", size = 1, alpha = 0.3) +
...
ctr <- pmx_nlmixr(fitOne.comp.KA.ODE_F, conts = c("WT","AGE"),cats=c("SEX","SPARSE"), vpc=FALSE,settings=pmx_settings(is.draft=FALSE))
...
xpdb.1F <- xpose_data_nlmixr(fitOne.comp.KA.ODE_F)
